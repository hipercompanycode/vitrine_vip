  ...
  return (
    <div className="rounded-2xl border border-line bg-surface p-4 shadow-card">
      ...
      <div className="mt-2 flex items-center gap-2">
        <button type="button" onClick={() => copy(link, "link")} className={`${btn} flex-1`}>{copied === "link" ? "Link copiado ✓" : "Copiar link"}</button>
        <a href={`https://wa.me/?text=${encodeURIComponent(msg)}`} target="_blank" rel="noopener noreferrer" className="inline-flex shrink-0 items-center gap-1.5 rounded-input bg-[#25D366] px-3 py-2 text-xs font-bold text-[#05340f] transition-opacity hover:opacity-90">
